...
# 51. Mysql中主从复制基于binLog完成，Mysql支持哪几种复制类型？

MySQL支持的三种主要复制类型基于binlog（二进制日志）格式的不同，分别是：

1. **基于语句的复制（Statement-Based Replication, SBR）**：
   - 这种复制方式记录了在主库上执行的所有更改数据的SQL语句，并将这些语句发送到从库进行重放。
   - 优点：相对节省磁盘空间和网络带宽，因为只需要传输实际的SQL命令。
   - 缺点：某些情况下，由于非确定性函数（如NOW()、UUID()等）的存在，可能导致主从不一致。
2. **基于行的复制（Row-Based Replication, RBR）**：
   - 基于行的复制不是记录整个SQL语句，而是记录每一行的变化。对于每个更改的数据行，都会记录一条对应的事件。
   - 优点：能够准确地复制数据变化，避免了SBR中可能遇到的不确定性和重复执行的问题。
   - 缺点：相比SBR，RBR会占用更多的存储空间和网络资源，因为它需要记录每一行的具体变更。
3. **混合模式复制（Mixed-Based Replication, MBR）**：
   - 混合模式复制结合了SBR和RBR的优点，在大多数情况下使用SBR以节约资源；但在遇到特定情况时（例如使用了非确定性函数），自动切换为RBR来保证数据的一致性。
   - 优点：提供了灵活性，能够在不同场景下选择最合适的复制方式。
   - 缺点：配置和维护可能比单一模式更复杂。

从MySQL 5.7.7开始，默认的binlog格式是`ROW`。这意味着除非用户明确指定了其他格式，否则新安装的MySQL实例将会采用row-based replication作为其默认设置来进行主从复制。这样的选择是因为row-based replication通常能提供更高的可靠性和一致性，尤其是在处理复杂的更新操作时。
