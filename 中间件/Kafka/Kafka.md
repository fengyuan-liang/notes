...
这里细心的同学可能会注意到，我们在手动进行`poll`消息的时候，是拿到一批消息`ConsumerRecords<String, String> records = consumer.poll(Duration.ofMillis(1000))`，但是上面的代码好像并没有去拿一批记录，这样是简写的写法，其实也可以拿一批记录
