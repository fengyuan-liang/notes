...
![JVM内存结构](https://cdn.fengxianhub.top/resources-master/202207091707720.png)
...
![image-20200704111417472](https://cdn.fengxianhub.top/resources-master/202207091515684.png)
...
![image-20200704112119729](https://cdn.fengxianhub.top/resources-master/202207091515610.png)
...
![image-20200704112700211](https://cdn.fengxianhub.top/resources-master/202207091516684.png)

C语言需要自己来分配内存和回收内存，Java全部交给JVM进行分配和回收。

### 1.4 推荐书籍

![image-20200704145340513](https://cdn.fengxianhub.top/resources-master/202207091516599.png)
...
![image-20200704151731216](https://cdn.fengxianhub.top/resources-master/202207091516734.png)

每个语言都需要转换成字节码文件，最后转换的字节码文件都能通过Java虚拟机进行运行和处理

![image-20200704152052489](https://cdn.fengxianhub.top/resources-master/202207091516730.png)
...
![image-20200704182035810](https://cdn.fengxianhub.top/resources-master/202207091517147.png)
...
![image-20200704183436495](https://cdn.fengxianhub.top/resources-master/202207091520887.png)

执行引擎包含三部分：解释器，及时编译器，垃圾回收器

### 2.7 Java代码执行流程

![image-20200704210429535](https://cdn.fengxianhub.top/resources-master/202207091520392.png)
...
由于Java需要热加载，会占用部分内存，对不需要长时间运行的，或者小型化的应用而言，Java（而不是指Java ME）天生就带有一些劣 势，特别是几年在从大型单体应用架构向小型微服务应用架构发展的技术潮流下，Java表现出来的不适应。

提前编译是解决Java占用内存高的方向之一，**提前编译**是相对于即时编译的概念，提前编译能带来的最大好处是Java虚拟机加载这些已经预编 译成二进制库之后就能够直接调用，而无须再等待即时编译器在运行时将其编译成二进制机器码。

理论上，**提前编译可以减少即时编译带来的预热时间**，减少Java应用长期给人带来的`第一次运行慢`的 不良体验，可以放心地进行很多全程序的分析行为，可以使用时间压力更大的优化措施。 

但是提前编译的坏处也很明显，它破坏了Java`一次编写，到处运行`的承诺，必须为每个不同的硬件、操作系统去编译对应的发行包；也显著降低了Java链接过程的动态性，必须要求加载的代码在 编译期就是全部已知的，而不能在运行期才确定，否则就只能舍弃掉已经提前编译好的版本，退回到原来的即时编译执行状态

**Substrate VM的出现**，算是满足了人们心中对Java提前编译的全部期待，运行在Substrate VM上的小规模应用， 其内存占用和启动时间与运行在HotSpot上相比有**5倍到50倍**的下降

>Substrate VM补全了Graal VM`Run Programs Faster Anywhere`愿景蓝图里的最后一块拼图，让 Graal VM支持其他语言时不会有重量级的运行负担
